...
def isGamePossible(redCount, greenCount, blueCount):
    games = 0
    for line in data:
        game = getGameId(line)
        tirage = getTirageData(line)
        if tirage[0] <= redCount and tirage[1] <= greenCount and tirage[2] <= blueCount:
            games += game
    return games
...
# print(getTirageData(data[4]))

print(isGamePossible(12, 13, 14))
